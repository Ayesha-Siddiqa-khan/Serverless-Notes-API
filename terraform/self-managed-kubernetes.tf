...
locals {
  postgres_backup_bucket_name = var.postgres_backup_bucket_name != "" ? var.postgres_backup_bucket_name : substr(lower(replace("${var.project_name}-${var.environment}-postgres-backups-${random_id.suffix.hex}", " ", "-")), 0, 63)
}
